...
Start Chla Analysis
##FORD- NEEDS NORMALIZED-change significance colors once done with trans
```{r}
#Chla MANOVA
...
summary(fordChlAman)
##NEEDS TRANSFORMED
ford1$logchla<-log(ford1$ChlA+1)
...
###STILL NEEDS TRANSFORMED
ford1$sqrtchla<-sqrt(ford1$ChlA)
fordsqrtman3<-aov(sqrtchla~nitrogen*phosphorus*trace, data=ford1)
shapiro.test(resid(fordsqrtman3))
par(mfrow=c(1,2))
plot(fordsqrtman3,which=c(1,2), main="Ford Normality")
summary(fordsqrtman3)
###sqrt and log transformations don't normalize these data...
#create plot to demonstrate ChlA~Treatment
par(mfrow=c(1,1))
ford1$treatment<-ford1$treatment[,drop=T]
plot(ChlA~treatment,data=ford1,
...
summary(neChlAman)
#LOG TRANSFORMATION
nerie1$logchla<-log(nerie1$ChlA+1)
...
###STILL NEEDS TRANSFORMED
nerie1$sqrtchla<-sqrt(nerie1$ChlA)
neriesqrtman3<-aov(sqrtchla~nitrogen*phosphorus*trace, data=nerie1)
shapiro.test(resid(neriesqrtman3))
par(mfrow=c(1,2))
plot(neriesqrtman3,which=c(1,2), main="NErie Normality")
summary(neriesqrtman3)
