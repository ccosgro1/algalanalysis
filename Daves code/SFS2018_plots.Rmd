...
GB.plot <- subset(inc3,inc3$Site=="GB")
...
```{r Maumee incubator N and trace only}
par(mai=c(1,1,0.2,1))

Mau.NT <- subset(Mau.plot,Mau.plot$Pho=="NoP"|Mau.plot$Treatment=="Initial")
Mau.NT$Treatment <- Mau.NT$Treatment[,drop=T]

plot(Chla*1000~Treatment,data=Mau.NT,las=2,xlab="",ylim=c(0,250),ylab="Chlorophyll a (µg/L)",cex.lab=1.5)
text(3,250,"WLE HAB 2017",cex=1.5)
points(MC*20~as.numeric(Treatment),data=Mau.NT,cex=2,col="red",lwd=2,pch=18)
axis(4,las=2,labels=seq(0,10,by=2),at=seq(0,250,by=50),col="red",col.axis="red")
mtext("Microcystin (µg/L)",4,line=2.5,col="red",cex=1.5)
abline(h=0.1*60,col="red",lty=2)

```


```{r Green Bay incubator N and trace only}
par(mai=c(1,1,0.2,1))

GB.NT <- subset(GB.plot,GB.plot$Pho=="NoP")
GB.NT$Treatment <- GB.NT$Treatment[,drop=T]
Mau.NT <- subset(Mau.plot,Mau.plot$Pho=="NoP")
Mau.NT$Treatment <- Mau.NT$Treatment[,drop=T]

plot(Chla*1000~Treatment,data=GB.NT,las=2,xlab="",ylim=c(0,250),ylab="Chlorophyll a (µg/L)",cex.lab=1.5,border="transparent")
plot(Chla*1000~Treatment,data=GB.NT,las=2,add=T,at=c(0.9,1.9,2.9,3.9),xaxt="n",yaxt="n",boxwex=0.3,ylab="",xlab="")
plot(Chla*1000~Treatment,data=Mau.NT,las=2,add=T,at=c(1.1,2.1,3.1,4.1),col="grey80",xaxt="n",yaxt="n",boxwex=0.3,xlab="",ylab="")

legend("topleft",c("Green Bay","Maumee"),fill=c("white","grey80"),bty="n")
points(2,238,lwd=2,col="red",cex=1.5)
points(2,250,col="red",cex=2,pch=18)
points(MC*20~c(1.1,4.1,2.1,3.1),data=GB.NT[c(1,5,9,13),],cex=2,col="red",lwd=2,pch=18)
points(MC*20~c(0.9,3.9,1.9,2.9),data=Mau.NT[c(1,5,9,13),],col="red",lwd=2,pch=1,cex=1.5)
axis(4,las=2,labels=seq(0,10,by=2),at=seq(0,250,by=50),col="red",col.axis="red")
mtext("Microcystin (µg/L)",4,line=2.5,col="red",cex=1.5)
abline(h=0.1*60,col="red",lty=2)
rect(-1,225,2.3,300)
